...
WELCOME_TEXT = (
    ...
    "🎟 בנוסף אפשר להזין קוד קופון בקופה.\n\n"
    "בחר מוצר מהתפריט:"
)
...
def main_menu_kb(context: Optional[ContextTypes.DEFAULT_TYPE] = None) -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton(
                    "💊 Kamagra Oral Jelly 100mg — 7 ערכות — ₪89",
                    callback_data="prod_kamagra",
                )
            ],
            [
                InlineKeyboardButton(
                    "🐎 Vidalista 40mg — 10 כדורים — ₪99",
                    callback_data="prod_vidalista",
                )
            ],
            [
                InlineKeyboardButton(
                    "💪 חבילת הגבר — Kamagra + Vidalista — ₪169",
...
def back_to_menu_kb() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([[InlineKeyboardButton("🔙 חזרה לתפריט", callback_data="menu")]])


def product_kb(key: str, context: ContextTypes.DEFAULT_TYPE) -> InlineKeyboardMarkup:
    rows: List[List[InlineKeyboardButton]] = []
    rows.append([InlineKeyboardButton("➕ הוסף לעגלה (בחר כמות)", callback_data=f"qty_{key}")])
    rows.append([InlineKeyboardButton("🛒 לעגלה", callback_data="cart")])
    ...
    rows.append([InlineKeyboardButton("🔙 חזרה לתפריט", callback_data="menu")])
...
def quantity_kb(key: str) -> InlineKeyboardMarkup:
    ...
    rows.append([InlineKeyboardButton("🔙 חזרה למוצר", callback_data=f"prod_{key}")])
    rows.append([InlineKeyboardButton("🔙 חזרה לתפריט", callback_data="menu")])
    return InlineKeyboardMarkup(rows)
...
def cart_kb(context: ContextTypes.DEFAULT_TYPE) -> InlineKeyboardMarkup:
    ...
    for key in cart.keys():
        p = PRODUCTS[key]
        rows.append([InlineKeyboardButton(f"❌ הסר {p['emoji']} {p['name']}", callback_data=f"rm_{key}")])

    if cart:
        rows.append([InlineKeyboardButton("🧹 נקה עגלה", callback_data="clearcart")])
        rows.append([InlineKeyboardButton("✅ המשך להזמנה", callback_data="checkout")])

    rows.append([InlineKeyboardButton("🛍 המשך לקניות", callback_data="menu")])
    return InlineKeyboardMarkup(rows)
...
def payment_kb() -> InlineKeyboardMarkup:
    rows = [[InlineKeyboardButton(opt, callback_data=f"pay_{opt}")] for opt in PAYMENT_OPTIONS]
    rows.append([InlineKeyboardButton("❌ ביטול הזמנה", callback_data="cancel_order")])
    rows.append([InlineKeyboardButton("🔙 חזרה לתפריט", callback_data="menu")])
    return InlineKeyboardMarkup(rows)
...
async def add_to_cart_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    text = (
        ...
        "רוצה להמשיך לקניות או לעבור לעגלה?"
    )

    kb = InlineKeyboardMarkup(
        [
            [InlineKeyboardButton("🛒 לעגלה", callback_data="cart")],
            [InlineKeyboardButton("🛍 המשך לקניות", callback_data="menu")],
        ]
    )
...
async def cart_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not cart:
        text = "🛒 העגלה שלך ריקה.\n\nבחר מוצר כדי להתחיל."
        await render_text_from_callback(update, context, text, reply_markup=back_to_menu_kb())
        return

    totals = cart_calculate_totals(cart)
    lines = cart_items_to_lines(cart)

    text = "🛒 *עגלת הקניות שלך*\n\n" + "\n".join(lines)
    text += (
        f"\n\n📦 סה\"כ חבילות: *{totals.total_packs}*"
        f"\n💊 סה\"כ כדורים: *{totals.total_pills}*"
        f"\n💰 ביניים: *₪{totals.subtotal}*"
    )
    if totals.auto_discount:
        text += f"\n🏷 הנחת כמות: *-₪{totals.auto_discount}*"
    text += f"\n💳 לתשלום: *₪{totals.total}*"
...
async def checkout_start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    cart = cart_get(context)
    if not cart:
        await render_text_from_callback(update, context, "העגלה ריקה — אין מה להזמין.", reply_markup=back_to_menu_kb())
    ...
    summary = (
        "🧾 *סיכום לפני קופה*\n\n"
        f"{items_compact_summary(cart)}\n\n"
        f"📦 סה\"כ חבילות: *{totals.total_packs}*\n"
        f"💊 סה\"כ כדורים: *{totals.total_pills}*\n"
        f"💰 לתשלום כרגע: *₪{totals.total}*\n\n"
        "✏️ מה השם המלא שלך?"
    )
...
async def get_name(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await update.message.reply_text("🏙 באיזו עיר אתה?", reply_markup=cancel_order_kb())
    return CITY


async def get_city(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    context.user_data["city"] = update.message.text.strip()
    await update.message.reply_text("📱 מה מספר הטלפון שלך?", reply_markup=cancel_order_kb())
    return PHONE
...
def build_order_from_context(update: Update, context: ContextTypes.DEFAULT_TYPE, payment_method: str) -> Dict[str, Any]:
    ...
    # Backward-ish compatibility fields (for older admin views)
    if items:
...
def order_user_summary(order: Dict[str, Any]) -> str:
    lines = [
        "✅ ההזמנה נקלטה!",
        "",
        f"🆔 מספר הזמנה: {order['order_id']}",
        "📦 פריטים:",
    ]
    for it in order.get("items", []):
        lines.append(
            f"- {it['product_emoji']} {it['product_name']} x{it['qty']} (₪{it['unit_price']} ליח')"
        )

    lines.append("")
    lines.append(f"📦 סה\"כ חבילות: {order.get('total_packs', 0)}")
    lines.append(f"💊 סה\"כ כדורים: {order.get('total_pills', 0)}")
    lines.append(f"💰 ביניים: ₪{order.get('subtotal', order['total'])}")
    if order.get("auto_discount"):
        lines.append(f"🏷 הנחת כמות: -₪{order['auto_discount']}")
    if order.get("coupon_discount"):
        lines.append(
            f"🎟 קופון {order.get('coupon_code')}: -₪{order['coupon_discount']}"
        )
    lines.append(f"💳 סה\"כ לתשלום: ₪{order['total']}")
    lines.append("")
    lines.append(f"👤 שם: {order['customer']['name']}")
    lines.append(f"🏙 עיר: {order['customer']['city']}")
    lines.append(f"📱 טלפון: {order['customer']['phone']}")
    lines.append(f"💳 תשלום: {order['payment_method']}")
    lines.append("")
    lines.append("נציג יצור איתך קשר בקרוב לסגירת ההזמנה. תודה 🙏")
    return "\n".join(lines)
...
async def finalize_order(
    ...
    # Forward proof to seller (if provided)
    ...
    await context.bot.send_message(
        chat_id=update.effective_user.id,
        text=(
            f"אם בא לך — אפשר לדרג עכשיו את חוויית ההזמנה עבור {order['order_id']}.\n"
            "זה עוזר לנו לשפר 🙌"
        ),
    ...
    # cleanup
    cart_clear(context)
    for k in ("name", "city", "phone", "coupon_code", "coupon_discount"):
...
async def payment_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    text = (
        "מעולה.\n\n"
        f"💳 תשלום: *{method}*\n\n"
        "כדי לאשר את ההזמנה — שלח עכשיו צילום מסך / תמונה כהוכחת תשלום."
    )
    await render_text_from_callback(
        update,
        context,
...
async def proof_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if not proof:
        await update.message.reply_text(
            "לא קיבלתי תמונה. בבקשה שלח צילום מסך/תמונה כהוכחת תשלום.",
            reply_markup=proof_kb(),
        )
        return PROOF

    method = context.user_data.get("payment_method") or ""
    await update.message.reply_text("✅ קיבלתי את ההוכחה. מעבד הזמנה...", reply_markup=back_to_menu_kb())
...
async def cancel_order_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    for k in ("name", "city", "phone", "coupon_code", "coupon_discount", "payment_method"):
        context.user_data.pop(k, None)
