...
try:
    from facebook.pixel import track_view_content, track_add_to_cart, track_purchase, track_lead
    _HAS_PIXEL = True
except ImportError:
    _HAS_PIXEL = False
...
# ─── Referral system ─────────────────────────────────────────
try:
    from bot_referral import create_referral_code, validate_referral_code, apply_referral_reward
    _HAS_REFERRAL = True
except ImportError:
    _HAS_REFERRAL = False
...
# ─── Analytics ────────────────────────────────────────────────
try:
    from bot_analytics import format_stats_report
    _HAS_ANALYTICS = True
except ImportError:
    _HAS_ANALYTICS = False

BOT_TOKEN = cfg.BOT_TOKEN
SELLER_CHAT_ID = cfg.SELLER_CHAT_ID
SELLER_USERNAME = cfg.SELLER_USERNAME
WHATSAPP_NUMBER = cfg.WHATSAPP_NUMBER

REMINDER_DELAY_SECONDS = cfg.REMINDER_DELAY_SECONDS
MAX_QTY = cfg.MAX_QTY
DISCOUNT_THRESHOLD = cfg.DISCOUNT_THRESHOLD
...
def save_json(path: str, data: Any) -> None:
    """Atomic JSON write — writes to a temp file then replaces, preventing corruption from race conditions."""
    import tempfile
...
MAX_INPUT_LENGTH = 100  # max chars for name, city, phone fields

def _sanitize_text(text: str) -> str:
    """Strip Markdown special chars from user input to prevent message injection."""
    # Remove chars that could break Telegram Markdown or inject formatting
    for ch in ("*", "_", "`", "[", "]", "~"):
        text = text.replace(ch, "")
    return text.strip()[:MAX_INPUT_LENGTH]


def _validate_name(text: str) -> Optional[str]:
    """Validate and sanitize a name field. Returns sanitized name or None if invalid."""
    name = _sanitize_text(text)
    if not name or len(name) < 2:
        return None
    # Only allow letters (any script), spaces, hyphens, apostrophes
    import re
    if not re.match(r"^[\w\s\-'.]{2,60}$", name, re.UNICODE):
        return None
    return name


def _validate_city(text: str) -> Optional[str]:
    """Validate and sanitize a city field. Returns sanitized city or None if invalid."""
    city = _sanitize_text(text)
    if not city or len(city) < 2:
        return None
    import re
    if not re.match(r"^[\w\s\-'.]{2,60}$", city, re.UNICODE):
        return None
    return city


def _validate_phone(text: str) -> Optional[str]:
    """Validate an Israeli phone number. Returns cleaned phone or None if invalid."""
    clean = text.strip().replace("-", "").replace(" ", "").replace("+", "")
    if not clean.isdigit() or len(clean) < 9 or len(clean) > 15:
        return None
...
def register_user(user) -> None:
    ...
    if existing:
        ...
        # Facebook Pixel — Lead event for new users
        if _HAS_PIXEL:
            try:
                track_lead(user_id=uid, first_name=user.first_name)
            except Exception:
                pass
...
def store_kb(context: Optional[ContextTypes.DEFAULT_TYPE] = None) -> InlineKeyboardMarkup:
    """Alias for main_menu_kb — used on /start to go straight to store."""
    return main_menu_kb(context)
...
def cart_kb(context: ContextTypes.DEFAULT_TYPE) -> InlineKeyboardMarkup:
    ...
    if cart:
        ...
        for key in cart:
            p = PRODUCTS[key]
            rows.append([
                InlineKeyboardButton(f"➕ עוד {p['emoji']}", callback_data=f"qty_{key}"),
                InlineKeyboardButton(f"🗑 הסר {p['emoji']}", callback_data=f"rm_{key}"),
            ])
        # Coupon field at cart level
        rows.append([InlineKeyboardButton("🎟 יש לי קוד קופון", callback_data="coupon_prompt")])
...
async def post_init(app: Application) -> None:
    ...
    commands = [
        ...
        BotCommand("referral", "קוד הפניה — חבר מביא חבר"),
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Go straight to store — skip splash screen step
...
async def product_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if key not in PRODUCTS:
        await render_text_from_callback(update, context, "❌ מוצר לא נמצא.", reply_markup=main_menu_kb(context))
        return
    p = PRODUCTS[key]
    bundle_upsell = ""
    if key == "bundle":
...
async def cart_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    totals = cart_calculate_totals(cart, coupon_code=context.user_data.get("coupon_code"))
    ...
    if totals.auto_discount:
        text += f"\n🏷 הנחה אוטומטית (3+): *-₪{totals.auto_discount}*"
    if totals.coupon_discount:
        text += f"\n🎟 קופון ({totals.coupon_code}): *-₪{totals.coupon_discount}*"
...
# ─── COUPON PROMPT IN CART ────────────────────────────────────


async def coupon_prompt_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Show coupon input prompt from within the cart screen."""
    query = update.callback_query
    await query.answer()
    await render_text_from_callback(
        update,
...
async def get_coupon_in_cart(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Handle coupon text typed while in cart coupon state."""
    code = (update.message.text or "").strip()
    if code.lower() in ("דלג", "skip", "no", "אין"):
        context.user_data.pop("coupon_code", None)
        context.user_data.pop("coupon_discount", None)
        await update.message.reply_text("קיבלתי. אין קופון.")
    ...
    if valid:
        ...
        await update.message.reply_text(f"✅ קוד {valid} הופעל — {pct}% הנחה! 🎉")
    else:
        await update.message.reply_text("❌ קוד לא תקין או לא קיים.")
    return ConversationHandler.END
...
async def checkout_start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    # Check if we have saved details from a previous order — offer to reuse
    saved_name = context.user_data.get("name")
    saved_city = context.user_data.get("city")
    saved_phone = context.user_data.get("phone")

    if saved_name and saved_city and saved_phone:
        summary = (
            "*סכום הזמנה*\n\n"
            f"{items_lines}\n\n"
            f"💳 סה\"כ מוצרים: *₪{totals.subtotal}*{discount_line}\n\n"
            "──────────────────\n"
            "👤 פרטים שמורים מהפעם הקודמת:\n"
            f"  שם: *{saved_name}*\n"
            f"  עיר: *{saved_city}*\n"
            f"  טלפון: *{saved_phone}*\n\n"
            "להמשיך עם אותם פרטים?"
        )
        reuse_kb = InlineKeyboardMarkup([
            [InlineKeyboardButton("✅ כן, המשך עם הפרטים האלו →", callback_data="checkout_reuse_details")],
            [InlineKeyboardButton("✏️ עדכן פרטים", callback_data="checkout_new_details")],
            [InlineKeyboardButton("✖️ ביטול", callback_data="cancel_order")],
        ])
        await render_text_from_callback(
        ...
        return DELIVERY  # skip name/city/phone — go straight to delivery
...
async def get_name(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    name = _validate_name(update.message.text)
    if not name:
        await update.message.reply_text("❌ השם לא תקין. נסה שוב (2-60 תווים, אותיות בלבד):", reply_markup=cancel_order_kb())
        return NAME
    context.user_data["name"] = name
    await update.message.reply_text("🏙 *שלב 2/3* — באיזו עיר גרים?", reply_markup=cancel_order_kb(), parse_mode=ParseMode.MARKDOWN)
    return CITY


async def get_city(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    city = _validate_city(update.message.text)
    if not city:
        await update.message.reply_text("❌ שם העיר לא תקין. נסה שוב (2-60 תווים):", reply_markup=cancel_order_kb())
        return CITY
    context.user_data["city"] = city
    await update.message.reply_text("📱 *שלב 3/3* — מספר טלפון לאישור ומעקב:", reply_markup=cancel_order_kb(), parse_mode=ParseMode.MARKDOWN)
    return PHONE
...
async def get_phone(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await update.message.reply_text(
        "🚚 *שלב 3/3* — איך תרצה לקבל את ההזמנה?",
...
async def checkout_reuse_details_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Use saved name/city/phone and skip to delivery step."""
    query = update.callback_query
    await query.answer()
    cart = cart_get(context)
    totals = cart_calculate_totals(cart)
    ...
    await render_text_from_callback(update, context, summary_text, reply_markup=delivery_kb(), parse_mode=ParseMode.MARKDOWN)
    return DELIVERY


async def checkout_new_details_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Clear saved details and start fresh from name input."""
    query = update.callback_query
    await query.answer()
...
async def _do_checkout(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if saved_name and saved_city and saved_phone:
        summary = (
            ...
            "──────────────────\n"
            "👤 פרטים שמורים:\n"
            f"  {saved_name} | {saved_city} | {saved_phone}\n\n"
            "להמשיך עם אותם פרטים?"
        )
        reuse_kb = InlineKeyboardMarkup([
            [InlineKeyboardButton("✅ כן, המשך →", callback_data="checkout_reuse_details")],
            [InlineKeyboardButton("✏️ עדכן פרטים", callback_data="checkout_new_details")],
            [InlineKeyboardButton("✖️ ביטול", callback_data="cancel_order")],
        ])
        await render_text_from_callback(update, context, summary, reply_markup=reuse_kb, parse_mode=ParseMode.MARKDOWN)
        return DELIVERY
...
async def delivery_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    cart = cart_get(context)
    totals = cart_calculate_totals(cart, coupon_code=context.user_data.get("coupon_code"))
    delivery_fee_now = context.user_data.get("delivery_fee", DELIVERY_COST)
    grand_total = totals.total + int(delivery_fee_now)
    items_lines = "\n".join([f"  {PRODUCTS[k]['emoji']} {PRODUCTS[k]['name']} ×{q}" for k,q in cart.items()])
    discount_line = f"\n🏷 הנחה: *-₪{totals.auto_discount}*" if totals.auto_discount else ""
    coupon_line = f"\n🎟 קופון: *-₪{totals.coupon_discount}*" if totals.coupon_discount else ""

    confirm_text = (
        "📋 *סיכום הזמנה — אישור לפני תשלום*\n\n"
        f"{items_lines}\n"
        f"{discount_line}{coupon_line}\n"
        f"📦 משלוח: *{label}*\n"
        f"\n💳 *סה\"כ לתשלום: ₪{grand_total}*\n\n"
        "──────────────────\n"
        "💰 בחר אמצעי תשלום:"
    )
    await render_text_from_callback(
...
async def finalize_order(
    ...
    if _HAS_PIXEL:
        try:
            ...
            logger.debug("Pixel Purchase event failed for %s", order.get("order_id"))

    # Apply referral reward if applicable
    if _HAS_REFERRAL:
        coupon_code = context.user_data.get("coupon_code", "")
        if coupon_code and coupon_code.startswith("REF-"):
            try:
                apply_referral_reward(coupon_code, update.effective_user.id, order.get("total", 0))
            except Exception:
                logger.debug("Referral reward failed for code %s", coupon_code)
...
async def referral_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Handle /referral — show or create a referral code for the user."""
    register_user(update.effective_user)
    if not _HAS_REFERRAL:
        await update.message.reply_text("מערכת ההפניות אינה זמינה כרגע.")
        return
    ...
    text = (
        "🎁 *תוכנית חבר מביא חבר*\n\n"
        f"הקוד שלך: `{code}`\n\n"
        "שלח את הקוד לחברים — כשהם ישתמשו בו בהזמנה,\n"
        "תקבל קופון הנחה אוטומטי! 🎉\n\n"
        "📋 שתף את הקוד ותתחיל להרוויח!"
    )
    await update.message.reply_text(text, parse_mode=ParseMode.MARKDOWN)
...
def main() -> None:
    ...
    checkout_handler = ConversationHandler(
        entry_points=[
            ...
            CallbackQueryHandler(checkout_reuse_details_callback, pattern=r"^checkout_reuse_details$"),
            CallbackQueryHandler(checkout_new_details_callback, pattern=r"^checkout_new_details$"),
        ],
        states={
    ...
    app.add_handler(CallbackQueryHandler(coupon_prompt_callback, pattern=r"^coupon_prompt$"))
    app.add_handler(CallbackQueryHandler(checkout_reuse_details_callback, pattern=r"^checkout_reuse_details$"))
    app.add_handler(CallbackQueryHandler(checkout_new_details_callback, pattern=r"^checkout_new_details$"))
