...
async def product_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if key not in PRODUCTS:
        await render_text_from_callback(update, context, "❌ מוצר לא נמצא.", main_menu_kb(), "Markdown")
        return
    p = PRODUCTS[key]
    caption = (
        f"{p['emoji']} *{p['name']}*\n\n"
...
async def get_name(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not name:
        await update.message.reply_text("❌ השם לא תקין. נסה שוב (2-60 תווים, אותיות בלבד):", reply_markup=cancel_kb())
        return NAME
...
async def get_city(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not city:
        await update.message.reply_text("❌ שם העיר לא תקין. נסה שוב (2-60 תווים):", reply_markup=cancel_kb())
        return CITY
...
async def get_phone(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not phone:
        await update.message.reply_text("❌ המספר לא נראה תקין. נסה שוב (לדוגמה: 0521234567)", reply_markup=cancel_kb())
        return PHONE
